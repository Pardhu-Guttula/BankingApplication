...
class TestAuthController(unittest.TestCase):
    ...
    def test_login_invalid_credentials(self, mock_auth_service):
        ...
        response = self.client.post('/login', json={"username": "test_user", "password": "test_wrong_pass"})
        self.assertEqual(response.status_code, 401)
        self.assertEqual(response.json["message"], "Invalid credentials")

    @patch('backend.authentication.controllers.auth_controller.auth_service')
    def test_login_no_data(self, mock_auth_service):
    ...
    def test_login_exception(self, mock_auth_service):
        mock_auth_service.authenticate.side_effect = Exception("Service error")
        response = self.client.post('/login', json={"username": "test_user", "password": "test_pass"})
        self.assertEqual(response.status_code, 500)
        self.assertEqual(response.json["message"], "Login failed")
    ...
    def test_register_success(self, mock_auth_service):
        ...
        response = self.client.post('/register', json={"name": "test_user", "email": "test@example.com", "password": "test_pass"})
    ...
    @patch('backend.authentication.controllers.auth_controller.auth_service')
    def test_register_no_data(self, mock_auth_service):
    ...
    def test_register_exception(self, mock_auth_service):
        mock_auth_service.register.side_effect = Exception("Service error")
        response = self.client.post('/register', json={"name": "test_user", "email": "test@example.com", "password": "test_pass"})
        self.assertEqual(response.status_code, 500)
        self.assertEqual(response.json["message"], "Registration failed")
